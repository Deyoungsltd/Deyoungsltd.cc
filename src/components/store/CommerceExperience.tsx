...
const electronicsProducts: Product[] = [
  { slug: "standing-fan-18inch", name: "18-Inch Standing Fan", description: "Heavy-duty standing fan with quiet 3-speed airflow for Nigerian homes.", price: 25000, category: "Home Appliances", image: "/products/fan.png", badge: "Featured" },
  ...
  { slug: "avr-stabilizer-2kva", name: "2KVA Automatic Voltage Regulator", description: "Reliable voltage protection for TVs, freezers, fridges, and office appliances.", price: 45000, category: "Voltage Protection", image: "/products/stabilizer.png", badge: "Power Safe" },
];

const boleProducts: Product[] = [
  { slug: "classic-bole-fish", name: "Classic Bole & Fish", description: "Roasted plantain served with grilled fish and signature pepper sauce.", price: 3500, category: "Bole Meals", image: "/products/bole_main.png", badge: "Customer Pick" },
  { slug: "roasted-yam-special", name: "Roasted Yam Special", description: "Roasted yam with rich palm oil sauce and perfectly grilled fish.", price: 3000, category: "Roasted Meals", image: "/products/bole_side.png" },
...
export function CommerceExperience({ business }: { business: Business }) {
  ...
  const { items: cart, addItem, updateQuantity, clearCart, getTotal } = useCart();
  ...
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      ...
      const res = await fetch(`/api/products?business=${business}&t=${cacheBuster}`, { cache: "no-store" });
      if (!res.ok) return;
      const data = await res.json();
      if (Array.isArray(data)) {
        setProductList(data.length > 0 ? data : copy.products);
      }
    }, 0);
    return () => window.clearTimeout(timer);
  }, [business, copy.products]);
    ...
    <main className={`min-h-screen bg-gradient-to-br ${copy.accent} text-white`}>
      <header className="sticky top-0 z-30 border-b border-white/10 bg-black/20 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5">
          ...
          <motion.button 
            ...
            className="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-bold backdrop-blur transition hover:bg-white/20"
          >
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 md:grid-cols-[1.05fr_.95fr]">
          <div className="relative z-10">
            <motion.p 
              ...
              className="text-xs font-bold uppercase tracking-[0.3em] text-white/55"
            >
              {copy.eyebrow}
            </motion.p>
            <motion.h1 
              ...
              className="mt-5 max-w-4xl font-display text-4xl font-black leading-[0.95] md:text-7xl"
            >
              {copy.title}
            </motion.h1>
            <motion.p 
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <motion.a 
                ...
                whileTap={{ scale: 0.95 }}
                href="#products" 
                className="rounded-full bg-white px-6 py-3 text-sm font-black text-stone-950 transition hover:shadow-2xl"
              >
                {copy.cta}
              </motion.a>
              <motion.a 
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                href={business === "electronics" ? "#repairs" : "#track"} 
          ...
          <div className="relative mx-auto h-[28rem] w-full max-w-lg overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 shadow-2xl backdrop-blur-sm">
            <HeroSceneLazy business={business} />
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-10">
        <div className="grid gap-4 md:grid-cols-3">
          {[{ icon: ShieldCheck, title: business === "electronics" ? "Power-safe buying" : "Fresh local prep", text: business === "electronics" ? "Products selected for real Nigerian power conditions." : "Roasted meals prepared for authentic Port Harcourt taste." }, { icon: PackageCheck, title: "Order tracking", text: "Every submitted order gets a tracking number for status updates." }, { icon: Copy, title: "Instant contact copy", text: "Phone and email details are quick-copy and WhatsApp-ready." }].map((item) => (
            <motion.div 
              whileHover={{ y: -5 }}
              className="rounded-3xl border border-white/10 bg-white/[0.07] p-6 backdrop-blur glass-card"
            >
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((category) => (
              <button 
                ...
                className={`rounded-full px-4 py-2 text-sm font-bold transition ${activeCategory === category ? "bg-white text-stone-950 shadow-lg" : "border border-white/15 text-white/70 hover:bg-white/10 hover:text-white"}`}
              >
                {category}
              </button>
            ))}
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {visibleProducts.map((product) => (
            <motion.article 
              key={product.slug} 
              whileHover={{ y: -10 }}
              className="group overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.08] shadow-2xl backdrop-blur glass-card"
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-5 flex items-center justify-between gap-3">
                  ...
                  <motion.button 
                    ...
                    className="rounded-full bg-white px-4 py-2 text-sm font-black text-stone-950 transition hover:bg-stone-100"
                  >
                    Add
                  </motion.button>
                </div>
      ...
      {business === "electronics" && (
        <section id="repairs" className="mx-auto max-w-7xl px-5 py-16">
          <div className="grid gap-8 rounded-[2rem] border border-white/10 bg-white/[0.07] p-6 backdrop-blur glass-panel md:grid-cols-[.9fr_1.1fr] md:p-10">
            <div>
            ...
            <form onSubmit={requestRepair} className="grid gap-3 md:grid-cols-2">
              ...
              <button className="md:col-span-2 rounded-2xl bg-white px-5 py-3 font-black text-stone-950 hover:bg-stone-100 transition-colors">Submit repair request</button>
      ...
      <section id="track" className="mx-auto max-w-7xl px-5 py-16 pb-28">
        <div className="grid gap-8 rounded-[2rem] border border-white/10 bg-black/20 p-6 backdrop-blur glass-dark md:grid-cols-2 md:p-10">
          <div>
          ...
          <form onSubmit={trackOrder} className="flex flex-col gap-3 sm:flex-row">
            ...
            <button className="rounded-2xl bg-white px-5 py-3 font-black text-stone-950 hover:bg-stone-100 transition-colors">Track</button>
          </form>
      ...
      <AnimatePresence>
        {checkoutOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-end justify-end bg-black/60 p-4 backdrop-blur-sm md:p-8"
          >
            <motion.div 
              ...
              className="max-h-[92vh] w-full max-w-xl overflow-auto rounded-[2rem] border border-white/10 bg-stone-950 p-6 text-white shadow-2xl glass-dark"
            >
              ...
              <div className="mt-5 flex items-center justify-between border-t border-white/10 pt-5"><span className="text-white/60">Total</span><strong className="text-xl">{formatNaira(getTotal())}</strong></div>
              <form onSubmit={placeOrder} className="mt-5 grid gap-3">
                ...
                <motion.button 
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="rounded-2xl bg-white px-5 py-3 font-black text-stone-950"
                >
                  <ShoppingCart className="mr-2 inline h-4 w-4" />Place order
                </motion.button>
